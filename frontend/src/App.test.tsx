...
describe('Maintenance Wizard dashboard', () => {
  ...
  it('shows only the supervisor LLM assistant to supervisor users', async () => {
    ...
    expect(screen.getByText('Risk: WO-8304 remains priority 1 and Approved.')).toBeInTheDocument()
    expect(screen.getByLabelText('Neo supervisor chat').textContent).not.toContain('APPR')
    expect(screen.getByText('LLM fallback · mock')).toBeInTheDocument()
  })
