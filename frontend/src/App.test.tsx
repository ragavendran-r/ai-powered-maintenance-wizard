...
describe('Intelligent Maintenance Wizard dashboard', () => {
  ...
  it('lets planners schedule and dispatch approved work orders without assistant panels', async () => {
    ...
    expect(within(pmPlanTable).queryByRole('button', { name: 'Active' })).not.toBeInTheDocument()
    fireEvent.click(within(pmPlanTable).getByRole('button', { name: 'PM-6999' }))
    expect(within(pmPanel).getByLabelText('Active preventive maintenance plan')).toHaveTextContent('Existing blower PM plan')
    const selectedPmPlanRow = within(pmPlanTable).getByRole('row', { name: /PM-6999/ })
    fireEvent.click(within(selectedPmPlanRow).getByRole('button', { name: 'Convert to planned work' }))
    await within(pmPanel).findByText(/Created WO-/)
    expect(await within(pmPlanTable).findByText('WO-9100')).toBeInTheDocument()
    const convertedPmPlanRow = within(pmPlanTable).getByRole('row', { name: /PM-6999/ })
    expect(within(convertedPmPlanRow).queryByRole('button', { name: 'Convert to planned work' })).not.toBeInTheDocument()
    expect(within(convertedPmPlanRow).getByLabelText('No PM plan action available')).toBeInTheDocument()
