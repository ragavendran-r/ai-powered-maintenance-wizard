...
- Role-based in-app notification events for work-order assignments, approvals, material readiness, dispatch, high/critical anomalies, and high-risk recommendations, with per-user seen/dismissed state, centered popups, a top-bar notification feed, LLM-generated next-action text when a live runtime is configured, deterministic fallback text, and Qdrant plant-record indexing for shared notification events.
