...
The AI layer is built as an auditable maintenance copilot, not an autonomous control loop. Raw IoT ingestion, anomaly scoring, risk calculation, role permissions, and persisted work-order updates stay in deterministic backend flows. AI is added after those checks to explain what is happening, retrieve plant-specific evidence, guide the right role through the next action, and turn reviewed outcomes into reusable learning material.

- Role-aware assistants: Neo adapts the assistant experience for operators, technicians, supervisors, engineers, reliability engineers, and admins. Technician mode focuses on step-by-step work-order execution, supervisor mode focuses on review/follow-up decisions, and admin mode can surface permitted user-management context. Morpheus supports diagnosis and recommendation generation, while Smith explains degradation, risk, and remaining-useful-life drivers.
- Grounded maintenance reasoning: AI prompts are assembled from current equipment state, active alerts, sensor/anomaly summaries, risk and RUL drivers, spare-part context, work-order history, retrieved evidence, and approved learning examples. The backend validates structured LLM JSON before merging it with deterministic recommendations, and falls back to deterministic reasoning when a provider is unavailable or returns invalid output.
- Evidence-first Retrieval-Augmented Generation (RAG): Uploaded SOPs, manuals, logs, maintenance history, document intelligence, approved feedback, completed work orders, and approved assistant interactions are chunked, embedded, and indexed. Production RAG uses Qdrant as the vector database, with SQLite-local vector scoring retained only as a fallback. Retrieval pulls both plant documents and approved learning examples into diagnosis, chat, reports, ranking, and recommendation prompts.
- RAG quality controls: Learning Review uses human approval plus an LLM-as-a-Judge rubric before feedback, labels, work-order outcomes, documents, or assistant interactions can become reusable examples. Judge-qualified examples are synchronized into Qdrant separately from source documents, preserving traceability and making it possible to reindex or migrate embedding profiles without mixing raw and reviewed knowledge.
- PEFT training and tuning path: The same approved, judge-qualified examples can be exported as JSONL dataset snapshots for parameter-efficient fine-tuning. Queued learning jobs write dataset and training-manifest artifacts with content hashes, optionally hand off to a configured external LoRA/QLoRA trainer, record trainer output, register adapter candidates, and require evaluation plus verified runtime deployment before promotion.
- Adapter lifecycle governance: PEFT outputs are treated as model-version candidates until reviewed. Learning Review tracks model versions, prompt versions, evaluation runs, promotion/rollback audit records, adapter runtime deployments, artifact storage status, and the active serving configuration used by OpenAI-compatible or Ollama-style providers.
- How RAG helps: RAG makes answers immediately sensitive to the current plant context without retraining a model. It reduces hallucination risk by grounding responses in manuals, logs, work orders, feedback, and approved examples, and it keeps evidence auditable because retrieved sources can be inspected and refreshed independently.
- How PEFT helps: PEFT lets the team specialize a smaller local model or SLM on curated steel-maintenance language, site-specific failure modes, status transitions, response style, and approved action patterns without retraining a base model. This can improve consistency, reduce prompt burden, and preserve local/offline operation while still requiring review gates before an adapter becomes active.
- Continuous improvement loop: New data flows from ingestion and daily work into reviewer-controlled learning. Useful outcomes improve short-term recommendations through RAG and ranking, then can improve longer-term model behavior through PEFT snapshots and adapter promotion once enough curated examples exist.
