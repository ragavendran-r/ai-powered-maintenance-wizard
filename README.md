...
Set `LEARNING_ARTIFACT_RETENTION_DAYS` above `0` to report expired local learning-artifact files in artifact-store lifecycle helpers and Learning Summary policy status. Cleanup remains dry-run by default; file deletion requires an explicit non-dry-run caller and `LEARNING_ARTIFACT_CLEANUP_ENABLED=true`.
...
The current SQLite schema version is `12`. Lightweight startup migrations add `feedback.equipment_id`, create asset detail tables, `document_intelligence`, `maintenance_labels`, `streaming_messages`, local auth tables, work orders, work-order logs, learning interactions, judged examples, dataset snapshots, model versions, prompt versions, evaluation runs, learning jobs, learning artifacts, and model promotion audit records for older local databases. Full migration tooling is still a production hardening item.
