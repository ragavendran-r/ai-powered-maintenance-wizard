...
IOT_SIMULATOR_INTERVAL_SECONDS="${IOT_SIMULATOR_INTERVAL_SECONDS:-5}"
IOT_SIMULATOR_ANOMALY_EVERY_SECONDS="${IOT_SIMULATOR_ANOMALY_EVERY_SECONDS:-120}"
IOT_SIMULATOR_SCENARIO="${IOT_SIMULATOR_SCENARIO:-mixed}"
IOT_SIMULATOR_ASSETS="${IOT_SIMULATOR_ASSETS:-all}"

BACKEND_LOG="${RUNTIME_DIR}/backend.log"
LEARNING_WORKER_LOG="${RUNTIME_DIR}/learning-worker.log"
IOT_SIMULATOR_LOG="${RUNTIME_DIR}/iot-simulator.log"
FRONTEND_LOG="${RUNTIME_DIR}/frontend.log"
BACKEND_PID_FILE="${RUNTIME_DIR}/backend.pid"
LEARNING_WORKER_PID_FILE="${RUNTIME_DIR}/learning-worker.pid"
IOT_SIMULATOR_PID_FILE="${RUNTIME_DIR}/iot-simulator.pid"
...
Commands:
  start   Start/reuse NATS JetStream, Qdrant, FastAPI backend, IoT simulator, learning worker, and Vite frontend.
  status  Show health for NATS, Qdrant, backend, IoT simulator, learning worker, streaming ingestion, learning, and frontend.
  stop    Stop backend/frontend/worker/simulator listeners and stop the named NATS/Qdrant containers.
...
wait_for_streaming_ingestion() {
  local attempts="${1:-40}"
  local token=""
  if ! token="$(demo_auth_token 2>/dev/null)"; then
    echo "Could not obtain demo admin token to check streaming ingestion; continuing." >&2
    return 0
  fi
  for _ in $(seq 1 "$attempts"); do
    ...
    case "$state" in
      connected)
        echo "Streaming ingestion is connected."
        return 0
        ;;
      disabled)
        echo "Streaming ingestion is disabled; simulator is not useful for ingestion."
        return 1
        ;;
      error)
        echo "Streaming ingestion reported error state; simulator start skipped." >&2
        return 1
        ;;
    esac
    sleep 0.5
  done
  echo "Streaming ingestion did not report connected state before simulator start." >&2
  return 1
}
...
start_iot_simulator() {
  if [[ "$IOT_SIMULATOR_ENABLED" != "true" ]]; then
    echo "IoT simulator disabled by IOT_SIMULATOR_ENABLED=${IOT_SIMULATOR_ENABLED}"
    return 0
  fi
  if pid_running "$IOT_SIMULATOR_PID_FILE"; then
    echo "IoT simulator already running: $(cat "$IOT_SIMULATOR_PID_FILE")"
    return 0
  fi
  if [[ ! -x "${BACKEND_DIR}/.venv/bin/python" ]]; then
    echo "Backend venv is missing. Run: cd backend && python3 -m venv .venv && source .venv/bin/activate && pip install -r requirements.txt" >&2
  ...
  if ! wait_for_streaming_ingestion; then
    return 0
  fi
  echo "Starting IoT sensor simulator with ${IOT_SIMULATOR_SCENARIO} scenario"
  (
    cd "$ROOT_DIR"
    "${BACKEND_DIR}/.venv/bin/python" scripts/publish-dummy-iot-readings.py \
      --nats-url "$NATS_URL" \
      --assets "$IOT_SIMULATOR_ASSETS" \
      --interval-seconds "$IOT_SIMULATOR_INTERVAL_SECONDS" \
      --anomaly-every-seconds "$IOT_SIMULATOR_ANOMALY_EVERY_SECONDS" \
      --scenario "$IOT_SIMULATOR_SCENARIO"
  ) >"$IOT_SIMULATOR_LOG" 2>&1 &
  echo "$!" >"$IOT_SIMULATOR_PID_FILE"
  STARTED_IOT_SIMULATOR=1
}
...
start_stack() {
  ...
  if [[ "$STARTED_BACKEND" -eq 1 || "$STARTED_LEARNING_WORKER" -eq 1 || "$STARTED_IOT_SIMULATOR" -eq 1 || "$STARTED_FRONTEND" -eq 1 ]]; then
    echo
    echo "Logs:"
    [[ "$STARTED_BACKEND" -eq 1 ]] && echo "  Backend:  ${BACKEND_LOG}"
    [[ "$STARTED_LEARNING_WORKER" -eq 1 ]] && echo "  Worker:   ${LEARNING_WORKER_LOG}"
    [[ "$STARTED_IOT_SIMULATOR" -eq 1 ]] && echo "  IoT sim:  ${IOT_SIMULATOR_LOG}"
...
wait_for_children() {
  ...
  [[ -f "$IOT_SIMULATOR_PID_FILE" ]] && iot_simulator_pid="$(cat "$IOT_SIMULATOR_PID_FILE")"
  ...
  while true; do
    ...
    if [[ -n "$iot_simulator_pid" ]] && kill -0 "$iot_simulator_pid" >/dev/null 2>&1; then
      any_running=1
    fi
...
stop_started() {
  ...
  stop_pid_file "$IOT_SIMULATOR_PID_FILE" "IoT simulator"
...
show_status() {
  ...
  echo "IoT simulator:"
  if pid_running "$IOT_SIMULATOR_PID_FILE"; then
    echo "ok: pid $(cat "$IOT_SIMULATOR_PID_FILE")"
  elif [[ "$IOT_SIMULATOR_ENABLED" == "true" ]]; then
    echo "not running"
  else
    echo "disabled"
  fi
