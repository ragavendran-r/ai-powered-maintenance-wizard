...
- Added a top-level README `AI Capabilities` section above `Current Capabilities`, covering role-aware assistants, grounded recommendation reasoning, Qdrant-backed RAG, Learning Review quality gates, PEFT dataset/training handoff, adapter lifecycle governance, and how RAG plus PEFT improve short-term and long-term maintenance AI behavior. Verification for this docs slice used README/progress re-read and `git diff --check`.
