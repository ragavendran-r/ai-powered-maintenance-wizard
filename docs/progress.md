...
- Added local PEFT artifact preparation: queued PEFT jobs now produce a JSONL dataset artifact and training manifest with SHA-256 hashes and `training_status=awaiting_external_peft_trainer`, giving external LoRA/QLoRA trainers an auditable handoff without claiming model training happened inside the web app.
...
- Updated README, architecture, RAG+PEFT+NATS design, and goal tracker to reflect the worker and artifact registry. Remaining G-016 work is external trainer/orchestrator execution, object storage, production embeddings/Qdrant migration controls, serving-runtime adapter activation, and Postgres migration.
...
- Add artifact storage for snapshots, training logs, and adapter outputs.
- Add PEFT worker/orchestrator execution for local Qwen/SLM LoRA or QLoRA training.
- Add production serving-runtime adapter activation after a reviewer promotes a passed model.
