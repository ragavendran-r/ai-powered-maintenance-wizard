...
- Qdrant RAG must index all plant-grounding records, not only uploaded documents: SOPs, manuals, logs, history, alerts, IoT messages, sensor readings, spares, work orders, PM plans, RCA records, reports, documents, active and inactive application users, assistant interactions, assistant-approved learning examples, and other persisted plant records. Direct SQLite lookup may supplement or degrade gracefully, but all LLM assistants should be able to retrieve grounded plant context through Qdrant when it is enabled.
...
- Assistant workflows must not hide chained LLM calls behind a spinner. If a chained LLM step is unavoidable and user-relevant, stream its readable output immediately; otherwise move it out of the pre-response path or replace it with deterministic context assembly that is clearly separate from assistant prose. Do not block the first visible assistant stream on non-streamed LLM reranking, explanation, judging, or planning calls.
- Assistant backends should use the shared Pydantic AI runtime contract: streamed text events, a structured final response event, persistent `session_id`, persisted message history, typed tool-call/tool-result events, validated output schemas, formatter-safe Markdown, and fallback only on explicit live-model failure. Legacy assistant routes may remain temporarily only while being migrated behind this contract.
