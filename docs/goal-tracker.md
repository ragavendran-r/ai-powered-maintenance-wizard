...
- Future production phases should move learning state to Postgres and harden large JSONL/model artifact storage with object-store lifecycle and access controls.
...
- Schema version 14 RAG embedding profile records, document chunk embedding metadata, active-profile retrieval filtering, Qdrant collection status/shape checks, reviewer-audited embedding profile activation, RAG reindex, and Qdrant migration preview/run controls.
- Approved, judge-qualified learning examples synchronized into Qdrant as separate RAG entries during learning refresh, approval changes, rejudge, and full RAG reindex flows.
