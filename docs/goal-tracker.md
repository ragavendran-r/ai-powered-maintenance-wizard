...
- `learning_artifacts` registry with content hashes for worker-produced PEFT dataset and training-manifest artifacts, backed by filesystem or S3-compatible object storage.
- Configurable external PEFT trainer command execution with bounded timeout, trainer logs, adapter manifests, and candidate model-version registration.
...
- Add external adapter registry/runtime deployment integration so PEFT outputs are loaded into LM Studio/Ollama or another serving runtime automatically.
