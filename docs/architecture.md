...
- Notification service: converts deterministic work-order, material, dispatch, high/critical anomaly, and high-risk recommendation events into role/user-targeted in-app notifications with per-user seen/dismissed state. LLM calls generate concise role-specific next-action text when a live provider is configured; deterministic templates are stored when the runtime is unavailable. Shared notification events are indexed as Qdrant plant records, while per-user read state remains SQLite-only.
