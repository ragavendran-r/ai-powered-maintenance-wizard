...
- Retrieval service: production Qdrant vector database for document chunks, approved knowledge, and indexed plant records, deterministic SQLite/local-vector fallback for tests or degraded disconnected operation, lexical scoring, optional LLM/SLM reranking, and relevance reasons.
...
- Notification service: converts deterministic work-order, material, dispatch, high/critical anomaly, and high-risk recommendation events into role/user-targeted in-app notifications with per-user seen/dismissed state. User actions persist immediately with deterministic next-action text; notification display requests return immediately and trigger non-blocking LLM enrichment for future polls when a live provider is configured. Shared notification events are indexed as Qdrant plant records, while per-user read state remains SQLite-only.
...
8. Chat or diagnosis requests trigger Qdrant-backed vector retrieval over persisted document chunks, approved learning examples, and indexed plant records such as alerts, sensor readings, spares, work orders, logs, RCA cases, PM plans, users, audit events, and learning records, with SQLite/local-vector fallback, embedding profile/version metadata, optional LLM/SLM reranking, and relevance reasons.
...
17. Production learning jobs publish to NATS JetStream for asynchronous judge, dataset, evaluation, PEFT, and adapter-deployment workers; workers persist job status, artifacts, metrics, candidate adapter versions, and runtime deployment health before any adapter is promoted. The PEFT worker prepares audited filesystem or S3-compatible artifacts and can invoke a configured external trainer with bounded timeout, then registers produced adapters as `candidate` adapter versions only. Adapter runtime deployment tracking/gating records whether a candidate is loaded for a serving target before active provider calls resolve it.
